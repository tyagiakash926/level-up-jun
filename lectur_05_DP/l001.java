...
public class l001{
    ...
    public static int boardPath_DP_01(int si, int ei, int[] dp) {
        for(si = ei;si>=0;si--)
        {
            ...
            int count = 0;
            for (int dice = 1; dice <= 6 && si + dice <= ei; dice++) {
                count += dp[dice+si]; //boardPath_DP_01(si + dice, ei, dp);
            }
    ...
    public static int friendsPairingProblem(int n,int[] dp)
    {
        ...
        int pairing = friendsPairingProblem(n-2,dp)*(n-1);
    ...
    public static int friendsPairingProblem_DP(int n,int[] dp)
    {
        ...
        for(n =0;n<=N;n++)
        {
            ...
            int single = dp[n-1];
            int pairing = dp[n-2]*(n-1);//friendsPairingProblem(n-2,dp)*(n-1);
            dp[n] = single+pairing;
        }
        return dp[N];
    }
    ...
    public static int  LIS_DP_LTR(int[] arr,int[] dp) {
        int max_=0;
    ...
    public static int  LDS_DP_RTL(int[] arr,int[] dp) {
        int max_=0;
    ...
    public static int targetSum_01_DP(int[] coins,int tar,int n,int[][] dp)
    {
        ...
        for(n=0;n<=N;n++)
        {
            for(tar=0;tar<=Tar;tar++)
            {
                ...
                dp[n][tar]=0;
                if(tar-coins[n-1]>=0)
                    dp[n][tar]+= dp[n-1][tar-coins[n-1]];//targetSum_01(coins,tar-coins[n-1],n-1,dp);
                dp[n][tar]+= dp[n-1][tar];//targetSum_01(coins,tar,n-1,dp);
            }
        }
        return dp[N][Tar];
    }
    ...
    // 0-1 knapshak // n isliye pass kia taki dp ka answer readable ho jaisi hum banate h agar vidx 0 paass krenge toh dp flow Unexpected aata h
    // isliye n pass kre h , isme n indicates n peeche se chalu hota h, 0 mei aage se code mei n ka use magar dry run 0 se aasanii hoti h kyuki
    // code mei n==0 bnaanna h  Dp ke liye;
    public static int knapshak_Rec(int[] weight,int[] arr,int cap,int n,int[][] dp)
    {
        if(n==0 || cap==0)
            return dp[n][cap] = 0;
        if(dp[n][cap]!=-1)
        ...
        int maxProfit =0;
        if(cap-arr[n-1]>=0)
            maxProfit = Math.max(maxProfit,knapshak_Rec(weight,arr,cap-arr[n-1],n-1,dp)+weight[n-1]);
        maxProfit = Math.max(maxProfit,knapshak_Rec(weight,arr,cap,n-1,dp));

        return dp[n][cap] = maxProfit;
    }
    
    public static int knapshak_Dp(int[] weight,int[] arr,int cap,int n,int[][]dp)
    {
        int N =n;
        int Cap = cap;
        for(n=0;n<=N;n++)
        {
            for(cap=0;cap<=Cap;cap++)
            {
                ...
                int maxProfit =0;
                if(cap-arr[n-1]>=0)
                    maxProfit = Math.max(maxProfit,dp[n-1][cap-arr[n-1]]+weight[n-1]);//knapshak_Rec(weight,arr,cap-arr[n-1],n-1,dp)+weight[n-1]);
                maxProfit = Math.max(maxProfit,dp[n-1][cap]);//knapshak_Rec(weight,arr,cap,n-1,dp));
    ...
    public static int knapshak_01_Rec(int[] weight,int[] arr,int cap,int n,int[][] dp)
    {
        if(n==0 || cap==0)
            return dp[n][cap] = 0;
        if(dp[n][cap]!=-1)
        ...
        int maxProfit =0;
        if(cap-arr[n-1]>=0)
            maxProfit = Math.max(maxProfit,knapshak_01_Rec(weight,arr,cap-arr[n-1],n,dp)+weight[n-1]);
        maxProfit = Math.max(maxProfit,knapshak_01_Rec(weight,arr,cap,n-1,dp));

        return dp[n][cap] = maxProfit;
    }

    public static int knapshak_01_Dp(int[] weight,int[] arr,int cap,int n,int[][]dp)
    {
        int N =n;
        int Cap = cap;
        for(n=0;n<=N;n++)
        {
            for(cap=0;cap<=Cap;cap++)
            {
                ...
                int maxProfit =0;
                if(cap-arr[n-1]>=0)
                    maxProfit = Math.max(maxProfit,dp[n][cap-arr[n-1]]+weight[n-1]);//knapshak_01_Rec(weight,arr,cap-arr[n-1],n,dp)+weight[n-1]);
                maxProfit = Math.max(maxProfit,dp[n-1][cap]);//knapshak_01_Rec(weight,arr,cap,n-1,dp));
    ...
    public static void knapshak()
    {
        int[] weights={1,4,5,7};
        int tar = 8;
        int[] value = {1,3,5,5};
        int[][] dp = new int[value.length+1][tar+1];
		for(int[] d:dp)
            Arrays.fill(d,-1);
        // System.out.println(knapshak_Rec(weights,value,tar,value.length,dp));
        // System.out.println(knapshak_Dp(weights,value,tar,value.length,dp));
        // System.out.println(knapshak_01_Rec(weights,value,tar,value.length,dp));
        System.out.println(knapshak_01_Dp(weights,value,tar,value.length,dp));
